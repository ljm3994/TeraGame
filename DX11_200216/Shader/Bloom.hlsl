...
float4 BlurV(PostProcessOut Input) : SV_Target
{
    return Blur(Input, float2(0, 1), 1.5);
}

float4 BlurH(PostProcessOut Input) : SV_Target
{
    return Blur(Input, float2(1, 0), 1.5);
}

float4 BloomThreshold(PostProcessOut Input) : SV_Target
{
    float4 color = InputTexture0.SampleLevel(samplerLinearClamp, Input.TextureUV, 0);
    color = saturate((color - 0.3) / (1 - 0.3f));
