...
Texture2D Tex : register(t3); // Color
Texture2D Tex1 : register(t4); // Depth
Texture3D TexLUT : register(t5); // Color correction
Texture2D TexAdaptedLuminance : register(t6); // Adapted luminance   
Texture2D TexVelocityBuffer : register(t7); // Velocity buffer
Texture2D TexBloom : register(t8);
...
{
	// 1단계: TexAdaptedLuminance 불러오고 최대 최소 값으로 clamp한다
...
float4 PS(in PostProcessOut Input) : SV_TARGET
{
    ...
    ColorBuffer.a = dot(ColorBuffer.rgb, float3(0.299, 0.587, 0.114));
